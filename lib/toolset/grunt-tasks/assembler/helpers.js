...
Helper = (function() {
  ...
  /**
   * Searchs for components (folders that contain package.yml files)
  ...
  Helper.prototype.getcomponentsData = function () {
    ...
    grunt.file.glob.sync(srcDir + '/components/**/package.yml').forEach(function (path) {
        var tplFile, modInfo = grunt.file.readYAML(path);
        // Don't parse angular components
        if (modInfo.type && modInfo.type === "angular") {
          return;
        }

        modInfo.tplFilePath = grunt.file.glob.sync(path.replace('package.yml', '*.jade'))[0];
        ...
        modInfo.identifier = modInfo.name;
        // grunt.log.writeln("getting component identifier", modInfo.identifier);
        grunt.log.writeln(chalk.cyan('\t\t— Found ') + chalk.yellow(modInfo.identifier));

        components.push(modInfo);
        allMetadata[modInfo.name] = modInfo;
    });
  ...
  Helper.prototype.getJadeFunctions = function () {
    var result = {};
    grunt.file.glob.sync(srcDir + '/**/*.jade.js').forEach(function (file) {
      try {
        var functions = require("../../../../" + file);
        _.each(functions, function(func, key) {
          result[key] = func;
        });
      }
      catch(e) {
  ...
  Helper.prototype.processComponentsData = function (componentsData) {
